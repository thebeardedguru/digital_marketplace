    ...
    <section className='max-w-7xl mx-auto px-4 lg:px-8 lg:grid lg:grid-rows-1 lg:grid-cols-7 lg:gap-x-8 lg:gap-y-10 xl:gap-x-16'>
        ...
        <CarouselContent>
          {data?.images.map((item, index) => (
            <CarouselItem key={index}>
              <div className='aspect-w-4 aspect-h-3 rounded-lg bg-gray-100 over-flow-hidden'>
                <Image
                  src={item as string}
                  alt='Product Image'
                  fill
      ...
      <div className='max-w-2xl mx-auto mt-5 lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3'>
        ...
        <div className='border-t border-gray-200 mt-10 pt-10'>
          <div className='grid grid-cols-2 w-full gapy-y-3'>
            ...
            <h3 className='text-sm font-medium col-span-1'>
              {getConditionType(data?.condition as string)?.title}
            </h3>
          </div>
        </div>
